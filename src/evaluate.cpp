...
namespace {
  ...
  const int ElephantSafeCheck = 780;
  const int HawkSafeCheck     = 780;
  ...
  Score Evaluation<T>::evaluate_pieces() {
    ...
    while ((s = *pl++) != SQ_NONE)
    {
        ...
        b = Pt == BISHOP ? attacks_bb<BISHOP>(s, pos.pieces() ^ pos.pieces(HAWK, QUEEN) ^ pos.pieces(Them, ELEPHANT))
          : Pt ==   ROOK ? attacks_bb<  ROOK>(s, pos.pieces() ^ pos.pieces(ELEPHANT, QUEEN) ^ pos.pieces(Us, ROOK))
  ...
  Score Evaluation<T>::evaluate_king() {
    ...
    if (kingAttackersCount[Them] > 0)
    {
        ...
        if ((b1 | b3) & attackedBy[Them][ELEPHANT] & safe)
            kingDanger += ElephantSafeCheck;

        // Enemy hawk safe checks
        if ((b2 | b3) & attackedBy[Them][HAWK] & safe)
            kingDanger += HawkSafeCheck;

        b1 &= attackedBy[Them][ROOK];
        b2 &= attackedBy[Them][BISHOP];
        b3 &= attackedBy[Them][KNIGHT];
        ...
        kingDanger +=       kingAttackersCount[Them] * kingAttackersWeight[Them]
                     ...
                     - 848 * !(pos.pieces(Them, QUEEN, ELEPHANT, HAWK) || pos.hand_score(Them))
                     -   9 * mg_value(score) / 8
                     +  40;
